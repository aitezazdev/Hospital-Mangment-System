...
import { Table, Button, Empty, Spin, Tabs, message } from "antd";
...
import { getAllAppointmentsForAdmin } from "../../apis/admin";
...
const AllAppointments = () => {
  ...
  const fetchAppointments = async () => {
    setLoading(true);
    try {
      const res = await getAllAppointmentsForAdmin();
  ...
  const filteredAppointments = appointments.filter((appt) => {
    ...
    const matchSearch = patientName.includes(search) || doctorName.includes(search);
  ...
  const handleApprove = async (id) => {
    try {
      ...
      message.error(error.message || "Error approving appointment");
    }
  };
  ...
  const handleCancel = async (id) => {
    try {
      ...
      message.error(error.message || "Error cancelling appointment");
    }
  };
  ...
  const handleComplete = async (id) => {
    try {
      ...
      message.error(error.message || "Error completing appointment");
    }
  };
